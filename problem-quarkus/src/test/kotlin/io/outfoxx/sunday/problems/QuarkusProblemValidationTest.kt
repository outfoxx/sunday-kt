...
class QuarkusProblemValidationTest {
  ...
  fun `generated problems decode with jackson`() {
    ...
    expectThat(decoded) {
      ...
      get { status }.isEqualTo(STATUS_CODE)
      get { detail }.isEqualTo(DETAIL)
      get { instance }.isEqualTo(INSTANCE)
      get { extra }.isEqualTo(EXTRA)
    }
  }
  ...
  fun `default title uses reason phrase for blank type`() {
    val problem = QuarkusProblemFactory.from(CUSTOM_STATUS).build()
    val adapter = QuarkusProblemFactory.adapter()
