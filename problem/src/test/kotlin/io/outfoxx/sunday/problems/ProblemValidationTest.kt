...
class ProblemValidationTest {
  ...
  fun `generated problems decode with jackson`() {
    ...
    expectThat(decoded.status).isEqualTo(STATUS_CODE)
  ...
  fun `registered problems decode as generated subclass`() {
    ...
    server.use {
      createRequestFactory(server).use { requestFactory ->
        ...
        expectThat(thrown.status).isEqualTo(STATUS_CODE)
  ...
  fun `default title uses reason phrase for blank type`() {
    val problem = SundayHttpProblem.Factory.from(CUSTOM_STATUS).build()
    val adapter = SundayHttpProblem.Factory.adapter()
  ...
  class GeneratedProblem(
    ...
    detail: String?,
    instance: URI?,
  ) : SundayHttpProblem(type, title, status, detail, instance) {

    val extra: String by extensions
  }
}
